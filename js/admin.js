...
(function () {
    ...
    var BXCCheckout = {
        ...
        embed: function (id = false) {
            ...
            for (var i = 0; i < index; i++) {
                ...
                if (id) {
                    let content = i == 0 ? `${BXC_CLOUD.custom_domain ? BXC_CLOUD.custom_domain : BXC_URL}${BXC_CLOUD.cloud ? '/checkout/' : (BXC_ADMIN_SETTINGS.url_rewrite_checkout ? BXC_ADMIN_SETTINGS.url_rewrite_checkout : 'pay.php?checkout_id=')}${id}${BXC_CLOUD.custom_domain ? '' : cloudURL()}` : (i == 1 ? `<div data-boxcoin="${id}"></div>${BXC_CLOUD.cloud ? ' <script id="boxcoin" src="' + BXC_URL + 'js/client.js?cloud=' + BXC_CLOUD.cloud + '"></script>' : ''}` : `[boxcoin id="${id}"]`);
    ...
    */
    ...
    function cloudURL() {
        return BXC_CLOUD && BXC_CLOUD.cloud && !BXC_CLOUD.custom_domain ? '&cloud=' + BXC_CLOUD.cloud : '';
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        */
        ...
        body.on('click', '#bxc-create-payment-link', function () {
            ...
            if (url) {
                url = `${BXC_CLOUD.custom_domain ? BXC_CLOUD.custom_domain : BXC_URL}${BXC_ADMIN_SETTINGS.url_rewrite_checkout ? BXC_ADMIN_SETTINGS.url_rewrite_checkout : 'pay.php?checkout_id='}custom-${Math.floor(Date.now() / 1000)}${cloudURL()}${url && BXC_ADMIN_SETTINGS.url_rewrite_checkout ? '?' + url.substring(1) : url}`;
