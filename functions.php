...
function bxc_transactions_invoice($transaction_or_id) {
    ...
    if (file_exists(__DIR__ . '/' . $path_part)) {
        return $invoice_url;
    }
...
function bxc_payment_link($transaction_id) {
    return bxc_get_url('id') . bxc_encryption($transaction_id);
}
...
function bxc_get_url($type, $full_url = true) {
    $custom_domain = BXC_CLOUD ? bxc_settings_get('custom-domain') : false;
    $cloud = BXC_CLOUD && !$custom_domain ? '&cloud=' . bxc_cloud_get_data() : '';
    $url_part = $custom_domain ? str_replace('//' . $type, '/' . $type, $custom_domain . '/' . $type . '/') : BXC_URL . (BXC_CLOUD ? $type . '/' : (bxc_settings_get('url-rewrite-' . $type) ? bxc_settings_get('url-rewrite-' . $type) : 'pay.php?' . ($type == 'checkout' ? 'checkout_id' : $type) . '='));
    return $full_url ? $url_part . $cloud : [$url_part, $cloud];
}
